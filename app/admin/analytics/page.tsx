...
function Bar({ value, max, color = "bg-[#0D6B30]" }: { value: number; max: number; color?: string }) {
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="p-8 max-w-5xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#0D6B30]" style={{ fontFamily: "var(--font-display)" }}>Analytics</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
        {[
          { label: "Total Articles", value: articles.length, icon: FileText, color: "text-[#0D6B30]", bg: "bg-[#0D6B30]/5" },
          { label: "Total Views", value: totalViews.toLocaleString(), icon: Eye, color: "text-blue-600", bg: "bg-blue-50" },
          { label: "Total Likes", value: totalLikes.toLocaleString(), icon: BarChart2, color: "text-[#C8831A]", bg: "bg-amber-50" },
          ...
          <div key={label} className="bg-white rounded-xl border border-slate-100 p-5">
            ...
            <p className="text-2xl font-bold text-[#0D6B30]">{value}</p>
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-6 mb-6">
        <div className="flex items-center gap-2 mb-5">
          <Eye size={16} className="text-[#0D6B30]" />
          <h2 className="font-bold text-[#0D6B30]" style={{ fontFamily: "var(--font-display)" }}>Most Viewed Articles</h2>
        </div>
          ...
          {topArticles.map((a, i) => (
            <div key={a.id} className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-[#0D6B30] truncate">{a.title}</p>
      ...
      <div className="bg-white rounded-xl border border-slate-100 p-6">
        <div className="flex items-center gap-2 mb-5">
          <BookOpen size={16} className="text-[#0D6B30]" />
          <h2 className="font-bold text-[#0D6B30]" style={{ fontFamily: "var(--font-display)" }}>Most Downloaded Resources</h2>
        </div>
          ...
          <div className="space-y-4">
            {topResources.map((r, i) => (
              <div key={r.id} className="flex items-center gap-4">
                <span className="text-xs font-bold text-slate-300 w-4 text-right flex-shrink-0">{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-[#0D6B30] truncate">{r.title}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <Bar value={r.downloads ?? 0} max={maxDownloads} color="bg-[#C8831A]" />
