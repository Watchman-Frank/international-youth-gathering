...
export default function AdminDashboardPage() {
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-12">
        {cards.map(({ href, icon: Icon, label, description, cta, color }) => (
          ...
          >
            ...
            <h2 className="font-bold text-[#0D6B30] text-sm mb-1.5" style={{ fontFamily: "var(--font-display)" }}>
              {label}
            </h2>
            <p className="text-xs text-slate-500 leading-relaxed mb-4">{description}</p>
            <span className="inline-flex items-center gap-1 text-xs font-bold text-[#0D6B30] group-hover:text-[#C8831A] transition-colors">
      ...
      {/* Tips */}
      <div className="bg-[#F0FAF3] rounded-xl border border-slate-100 p-6">
        <h3 className="font-bold text-[#0D6B30] text-sm mb-3">How to use uploads on your site</h3>
        <ol className="space-y-2 text-sm text-slate-600">
          <li className="flex gap-2.5">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#0D6B30] text-white text-[10px] font-bold flex items-center justify-center mt-0.5">1</span>
            Go to <strong>Media Library</strong> and upload your file.
          </li>
          <li className="flex gap-2.5">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#0D6B30] text-white text-[10px] font-bold flex items-center justify-center mt-0.5">2</span>
            Click <strong>Copy URL</strong> next to the uploaded file.
          </li>
          <li className="flex gap-2.5">
            <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#0D6B30] text-white text-[10px] font-bold flex items-center justify-center mt-0.5">3</span>
