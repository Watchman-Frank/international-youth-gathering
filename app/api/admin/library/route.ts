...
async function authorized(req: NextRequest, session: Session | null) {
  return (await checkAdminAuth(req)) || isSuperAdmin(session);
}

export async function GET(request: NextRequest) {
  const session = (await auth()) as Session | null;
  if (!await authorized(request, session)) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
