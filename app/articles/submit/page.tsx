...
export default function SubmitArticlePage() {
  ...
  if (status !== "loading" && !session) {
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-20 text-center">
        <div className="w-16 h-16 rounded-full bg-[#0D6B30]/10 flex items-center justify-center mx-auto mb-5">
          <Lock size={28} className="text-[#0D6B30]" />
        </div>
        <h1 className="text-2xl font-bold text-[#0D6B30] mb-2" style={{ fontFamily: "var(--font-display)" }}>
          Sign In to Submit
        </h1>
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 px-6 py-3 bg-[#0D6B30] text-white text-sm font-bold rounded-xl hover:bg-[#0A5423] transition-colors"
        >
          Sign In / Create Account
        </Link>
        <div className="mt-4">
          <Link href="/articles" className="text-sm text-slate-500 hover:text-[#0D6B30]">
            ← Back to Articles
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        <div className="bg-green-50 border border-green-200 rounded-2xl p-10 text-center">
          ...
          <h2 className="text-xl font-bold text-[#0D6B30]" style={{ fontFamily: "var(--font-display)" }}>Submission Received!</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-6">
            <Link href="/articles" className="px-5 py-2.5 bg-[#0D6B30] text-white text-sm font-semibold rounded-xl hover:bg-[#0A5423] transition-colors">
              Back to Articles
            </Link>
            <button onClick={() => { setSubmitted(false); setPdfFiles([]); setTitle(""); setSummary(""); setCategory(""); }} className="px-5 py-2.5 border border-slate-200 text-[#0D6B30] text-sm font-semibold rounded-xl hover:bg-slate-50 transition-colors">
              Submit Another
            </button>
          </div>
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
      <Link href="/articles" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-[#0D6B30] mb-8 transition-colors">
        <ArrowLeft size={14} aria-hidden /> Back to Articles
      </Link>

      {/* Author identity (from session) */}
      <div className="flex items-center gap-3 mb-6 p-4 bg-[#F0FAF3] rounded-xl border border-slate-100">
        {session?.user?.image && (
          <img src={session.user.image} alt="" className="w-10 h-10 rounded-full object-cover" />
        )}
        <div>
          <p className="text-sm font-bold text-[#0D6B30]">{session?.user?.name}</p>
          <p className="text-xs text-slate-500">{session?.user?.email}</p>
        </div>
      </div>

      <h1 className="text-3xl font-bold text-[#0D6B30] mb-2" style={{ fontFamily: "var(--font-display)" }}>Submit an Article</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label htmlFor="title" className="block text-sm font-semibold text-[#0D6B30] mb-1.5">Article Title <span className="text-red-500">*</span></label>
          <input id="title" required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="A compelling title for your article" className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#C8831A]" />
        </div>

        <div>
          <label htmlFor="category" className="block text-sm font-semibold text-[#0D6B30] mb-1.5">Category <span className="text-red-500">*</span></label>
          <select id="category" required value={category} onChange={(e) => setCategory(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#C8831A]">
        ...
        <div>
          <p className="text-sm font-semibold text-[#0D6B30] mb-2">How would you like to submit? <span className="text-red-500">*</span></p>
          <div className="flex flex-wrap gap-2">
            {(["text", "pdf", "both"] as const).map((type) => (
              <button key={type} type="button" onClick={() => setSubmissionType(type)} aria-pressed={submissionType === type}
                className={cn("px-4 py-2 rounded-xl text-sm font-semibold border transition-all", submissionType === type ? "bg-[#0D6B30] text-white border-[#0D6B30]" : "bg-white text-slate-600 border-slate-200 hover:border-slate-300")}>
        ...
        {showTextArea && (
          <div>
            <div className="flex items-start justify-between gap-3 mb-1.5">
              <label htmlFor="body" className="block text-sm font-semibold text-[#0D6B30]">Article Summary {submissionType === "text" && <span className="text-red-500">*</span>}</label>
            ...
            <div className="flex items-start gap-2 mb-2 text-xs text-slate-500 bg-[#F0FAF3] border border-slate-200 rounded-xl px-3 py-2.5">
              <Info size={13} className="text-[#C8831A] mt-0.5 flex-shrink-0" />
            ...
            <textarea id="body" required={submissionType === "text"} rows={5} value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="e.g. This article explores what it means to carry the Apostolic Mandate in the age of social media…" className={cn("w-full px-4 py-3 rounded-xl border text-sm resize-none focus:outline-none focus:ring-2 focus:border-transparent leading-relaxed", countWords(summary) > WORD_LIMIT ? "border-red-300 focus:ring-red-300" : "border-slate-200 focus:ring-[#C8831A]")} />
        ...
        {showPdfUpload && (
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-sm font-semibold text-[#0D6B30]">Upload PDF{submissionType === "both" ? " (optional)" : ""}{submissionType === "pdf" && <span className="text-red-500"> *</span>}</label>
            ...
            <div role="button" tabIndex={0} onClick={() => fileInputRef.current?.click()} onKeyDown={(e) => e.key === "Enter" && fileInputRef.current?.click()} onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop}
              className={cn("flex flex-col items-center justify-center gap-3 px-6 py-10 rounded-2xl border-2 border-dashed cursor-pointer transition-all", dragging ? "border-[#C8831A] bg-amber-50" : "border-slate-200 bg-slate-50 hover:border-[#0D6B30] hover:bg-[#F0FAF3]")}>
              <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center", dragging ? "bg-[#C8831A]" : "bg-white border border-slate-200")}>
                <Upload size={22} className={dragging ? "text-[#0D6B30]" : "text-slate-400"} />
              </div>
              <div className="text-center">
                <p className="text-sm font-semibold text-[#0D6B30]">{dragging ? "Drop your PDF here" : "Click to browse or drag & drop"}</p>
              ...
              <ul className="mt-3 space-y-2">
                {pdfFiles.map(({ name, sizeLabel, error }) => (
                  <li key={name} className={cn("flex items-center gap-3 px-4 py-3 rounded-xl border text-sm", error ? "bg-red-50 border-red-200" : "bg-white border-slate-200")}>
                    {error ? <AlertCircle size={18} className="text-red-500 flex-shrink-0" /> : <FileText size={18} className="text-[#C8831A] flex-shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <p className={cn("font-medium truncate", error ? "text-red-700" : "text-[#0D6B30]")}>{name}</p>
        ...
        <button type="submit" disabled={submitting} className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-[#0D6B30] text-white font-bold text-sm rounded-xl hover:bg-[#0A5423] transition-colors disabled:opacity-60">
