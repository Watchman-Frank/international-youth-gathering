  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-16">
      ...
      <section aria-labelledby="conf-heading">
        <div className="bg-gradient-to-br from-[#0D6B30] to-[#083D1C] rounded-3xl overflow-hidden">
          {upcoming?.flyer && (
            <div className="relative h-60 sm:h-80 overflow-hidden">
              <img src={upcoming.flyer} alt="God-Life Conference 2026" className="w-full h-full object-cover opacity-30" />
              <div className="absolute inset-0 bg-gradient-to-b from-transparent to-[#0D6B30]" />
            </div>
          )}
            ...
            {upcoming && (
              <div className="flex flex-wrap gap-x-6 gap-y-2 mt-6 text-sm text-white/60">
                <span className="flex items-center gap-2">
                  <Calendar size={15} className="text-[#C8831A]" aria-hidden />
                  August 14–17, 2026
                </span>
                {upcoming.location && (
                  <span className="flex items-center gap-2">
                    <MapPin size={15} className="text-[#C8831A]" aria-hidden />
                    {upcoming.location}
                  </span>
                )}
                <span className="flex items-center gap-2">
                  <Globe size={15} className="text-[#C8831A]" aria-hidden />
                  Streaming Worldwide
                </span>
              </div>
            )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            ...
            <div key={title} className="bg-white rounded-2xl p-6 border border-slate-100 hover:shadow-md transition-all">
              ...
              <h3 className="font-bold text-[#0D6B30] mb-2" style={{ fontFamily: "var(--font-display)" }}>{title}</h3>
      ...
      {/* Core Values strip */}
      <section className="bg-[#F0FAF3] rounded-2xl p-8 grid grid-cols-2 sm:grid-cols-4 gap-6 text-center" aria-label="IYG Core Values">
        {[
          ...
          <div key={value}>
            <div className="text-lg font-bold text-[#0D6B30]" style={{ fontFamily: "var(--font-display)" }}>{value}</div>
      ...
      <section aria-labelledby="past-heading">
        <div className="border-l-[3px] border-[#C8831A] pl-4 mb-6">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#C8831A] mb-1">On Demand</p>
          <h2 id="past-heading" className="text-2xl font-bold text-[#0D6B30]" style={{ fontFamily: "var(--font-display)" }}>
