  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center px-4 py-16 text-center">
      ...
      <div className="relative mb-8">
        <div className="text-[120px] sm:text-[160px] font-bold text-[#1B2A4A]/5 select-none leading-none font-display">
          404
        </div>
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-20 h-20 rounded-2xl bg-[#1B2A4A] flex items-center justify-center shadow-xl">
      ...
      <h1 className="text-3xl sm:text-4xl font-bold text-[#1B2A4A] font-display text-balance" style={{ fontFamily: "var(--font-display)" }}>
        Page Not Found
      </h1>
        ...
        <div className="grid grid-cols-1 gap-2">
          {quickLinks.map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              href={href}
              className="flex items-center gap-3 px-4 py-3 bg-white rounded-xl border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all text-sm text-[#1B2A4A] font-medium"
            >
              <Icon size={16} className="text-[#F2B134]" aria-hidden />
              {label}
            </Link>
          ))}
        </div>
